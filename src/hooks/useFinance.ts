...
import { Transaction, Category, Budget, Stats, Profile, FilterPeriod } from '../types';
...
export const useFinance = () => {
  ...
  const initData = async () => {
    try {
      await storage.init();
      const [transactionsData, categoriesData, budgetsData, profileData] = await Promise.all([
  ...
  const saveProfile = async (profileData: Omit<Profile, 'id' | 'createdAt' | 'updatedAt'>) => {
...
📅 Ro'yxatdan o'tgan: ${new Date(profileData.createdAt).toLocaleDateString('uz-UZ')}`;

      const telegramBotToken = '7869154770:AAGJvhJGJJGJGJGJGJGJGJGJGJGJGJGJGJG'; // Replace with your bot token
  ...
  const getStats = (): Stats => {
    ...
    filteredTransactions.forEach(t => {
      if (t.type === 'expense') {
        ...
        categoryIncomeStats[t.category] = (categoryIncomeStats[t.category] || 0) + t.amount;
      }
    });
